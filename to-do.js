...
//TRAVEL BAG ------CONSTRUCTOR
function TravelBag(){

	this.update = function(note = "") {
	console.log(`--- ${note}`);
	console.log("Travel Bag: ", this.bag);
	}
	...
	this.remove = function(itemId) {
		this.update(`Removed ${this.bag[itemId].item}`);
		this.bag.splice(itemId, 1);
	};

	this.packed = function(itemId) {
		this.bag[itemId].packed = true;
		this.update(`Packed ${this.bag[itemId].item}`);
	};

	this.change = function(itemId, newItem = "") {
		this.bag = this.bag.map( function(bag){
			if(bag.itemId === itemId){

				// this.update(`Changed ${bag[itemId].item}`);
				// console.log(this.bag);
				this.update( `changed ${bag.item} to ${newItem}`);

				return{...bag, item:newItem};
...
data.setItem("hello", "there");

data.setItem(travelBag);

const travelBag = new TravelBag();
travelBag.add("toothbrush");
travelBag.add("meds");
travelBag.add("socks");
travelBag.add("shirt");
travelBag.add("shoes");
travelBag.packed(1);
travelBag.change(1, "something");
travelBag.change(3, "something");
travelBag.remove(0);

const foodOrders = new TravelBag();
...
/*
	1. use the built-in Array method Array.forEach()
	2. display each name of the fruit 
	3. but also the index of that fruit to the left of it
	4. but make sure that it starts with 1 instead of 0
*/

// vegetablesData.data.forEach( function(obj, index) {
// 	var name = obj.name;
// 	var id = index;
// 	id++;
// 	var idName = `${id}---${name}`; 
...
/*
	1. use the built-in Array method Array.filter()
	2. display onlythe items from the family "Rosaceae"
	3. but also the index of that fruit to the left of it
	4. and make sure that it starts with 1 instead of 0

	5. try a bunch of filters! 
      What about trying to filter things based on many property matches?
*/

// vegetablesData.data.filter(function(object, index){
// 	if(object.family === "Rosaceae"){
// 		var vegId = index + 1;
// 		var vegSpecific = object;
// 		var name = object.name;
// 		var IdFam = `${vegId} --- ${name}`
// 		console.log(IdFam);
// 		console.log(vegSpecific);
// 	}
// });
